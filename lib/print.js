...
export default async function (m, conn = { user: {} }) {
    ...
    console.log(`╭────────────────────────────────···
│💚  ${chalk.greenBright('%s')}\n│⏰ㅤ${chalk.greenBright(oraCol)}\n│📑ㅤ${chalk.greenBright('%s')}\n│📊ㅤ${chalk.greenBright('%s [%s %sB]')}
│🗣ㅤ${chalk.greenBright('%s')}\n│📃ㅤ${chalk.greenBright('%s%s')}\n│🌐ㅤ${chalk.greenBright(chatName)}\n│📝ㅤ${chalk.greenBright('%s')}
╰───────────────────···
`.trim(),
    ...
    if (typeof m.text === 'string' && m.text) {
        ...
        if('@' + user.split`@`[0], chalk.greenBright('@' +await conn.getName(user)));
        console.log(m.error != null ? chalk.greenBright(log) : m.isCommand ? chalk.greenBright(log) : log);
    }
    if (m.messageStubParameters) {
        console.log(m.messageStubParameters.map(jid => {
            ...
            return name ? chalk.greenBright(`${phoneNumber} (${name})`) : ''
        }).filter(Boolean).join(', '))
    }
    if (/document/i.test(m.mtype)) console.log(chalk.greenBright(`🗂️ ${m.msg.fileName || m.msg.displayName || 'Document'}`))
    else if (/ContactsArray/i.test(m.mtype)) console.log(chalk.greenBright(`👨‍👩‍👧‍👦 ${' ' || ''}`))
    else if (/contact/i.test(m.mtype)) console.log(chalk.greenBright(`👨 ${m.msg.displayName || ''}`))
    else if (/audio/i.test(m.mtype)) {
        const duration = m.msg.seconds
        console.log(chalk.greenBright(`${m.msg.ptt ? '🎤ㅤ(PTT ' : '🎵ㅤ('}AUDIO) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`))
    }
    console.log()
}
let file = global.__filename(import.meta.url)
watchFile(file, () => {
    console.log(chalk.greenBright("Update 'lib/print.js'"))
})